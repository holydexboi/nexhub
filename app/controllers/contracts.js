...
router.post(
  "/create",
  authMiddleware(["superAdmin", "user"]),
  (req, res, next) => {
    uploadDirect.fields([{ name: "productPic", maxCount: 4 }])(
      req,
      res,
      (err) => {
        if (
  ...
  async (req, res) => {
    ...
    const existingExporter = await User.findOne({
      _id: exporterId,
      userType: "exporter",
    });
    if (!existingExporter)
...
router.put(
  ...
  async (req, res) => {
    ...
    if (status === "approved" || status === "rejected") {
      ...
      existingContract.isContractApproved =
      ...
      const successMessage =
        status === "approved"
          ? CONTRACT_CONSTANTS.CONTRACT_APPROVED
          : CONTRACT_CONSTANTS.CONTRACT_REJECTED;
